---
name: api-doc-output
description: 改动 HTTP/RPC 接口时强制同步更新接口文档——在 docs/api/<service>.md 记录路径、方法、入参/出参、错误码、鉴权要求。
...
改接口必须同步更新 `docs/api/<service>.md`，否则 CI 拦截。
...
```
docs/api/<service>.md       ← 后端 HTTP/gRPC 接口
  services/api/             → docs/api/api.md
  services/matching-engine/ → docs/api/matching-engine.md

docs/api/web.md             ← 前端对后端的接口依赖说明（可选）
```
